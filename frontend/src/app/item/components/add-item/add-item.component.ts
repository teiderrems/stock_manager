...
export class AddItemComponent implements OnInit {
  ...
  constructor(
    ...
    private readonly messageSertvice: NzMessageService
  ) {}
  ...
  startShowMessages(): void {
    this.messageSertvice
      ...
      .onClose!.pipe(
        concatMap(
          () =>
            this.messageSertvice.success("Loading finished", {
              nzDuration: 2500,
              nzAnimate: this.isSubmit(),
            }).onClose!
        ),
        concatMap(
          () =>
            this.messageSertvice.info("Loading finished is finished", {
              nzDuration: 2500,
            }).onClose!
        )
      )
